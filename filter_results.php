...
<?php
  ...
  $max_price = (int)$_GET['max_price'];
            ...
            $sql = "SELECT * FROM product_details WHERE `product_new_price` BETWEEN $min_price AND $max_price
            AND status = 'approved' AND `balanced_stock`!=0
             ORDER BY `product_new_price` ASC";
            // $sql = "SELECT * FROM `product_details`
            // JOIN `user_details` ON `product_details`.`user_id`=`user_details`.`user_id`  ORDER BY `product_details`.`user_id`";
