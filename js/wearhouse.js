$(document).ready(function(){
              ...
              $.each(data, function(key, value){ 
                $(".fetch_cart").append('<tr>'+
                                    '<td class="text-center product-thumbnail">'+
                                        '<a class="text-reset" href="#"><img src="upload/wearhouse/'+value.image+'" class="img-fluid" width="100" alt=""></a>'+
                                    '</td>'+
                                    '<td class="text-center product-name"><a class="text-reset" href="#">'+value.product_name+'</a></td>'+
                                    '<td class="text-center product-price-cart"><span class="amount">'+value.product_price+'</span></td>'+
                                    '<td class="text-center product-quantity">'+
                                        ...
                                        '</div>'+
                                    '</td>'+
                                    '<td class="text-center product-subtotal">'+value.total+'</td>'+
                  ...
                  $(".fetch_Order_cart").append('<tr>'+
                                    '<td class="text-center product-thumbnail">'+
                                        '<a class="text-reset" href="#"><img src="https://www.purie.in/upload/wearhouse/'+value.image+'" class="img-fluid" width="100" alt=""></a>'+
                                    '</td>'+
                                    '<td class="text-center product-name"><a class="text-reset" href="#">'+value.product_name+'</a></td>'+
                                    '<td class="text-center product-price-cart"><span class="amount">'+value.price+'</span></td>'+
                  ...
                  $(".side_bar_cart").append('<li class="py-3 border-bottom">'+
                                '<div class="row align-items-center">'+
                                    '<div class="col-4">'+
                                        '<img class="img-fluid border" src="https://www.purie.in/upload/wearhouse/'+value.image+'" alt="...">'+
                                    '</div>'+
                                    '<div class="col-8">'+
                                        '<p class="mb-2">'+
                                            '<a class="text-dark fw-500" href="#">'+value.product_name+'</a>'+
                                            '<span class="m-0 text-muted w-100 d-block">'+value.price+'.00 x '+value.qty+'</span>'+
                                            '<span class="m-0 text-muted w-100 d-block">Total : '+value.total+'.00</span>'+
                                        '</p>'+
                                    '</div>'+
                                '</div>'+
                            '</li>');
    ...
    function loadAllCartCount(){ 
        ...
        $.ajax({ 
          ...
          success : function(data){
            var subPrice=0;
            var pro_price=data.product_price;
            subPrice += pro_price;
              // console.log(data);
            // if(data.status == true){
            //   $(".fetch_cart_count").append('0');
            //   $(".fetch_cart_value").append('');
              $(".fetch_cart_value_sub").html(subPrice);
            ...
            // }else{
              ...
              $.each(data, function(key, value){ 
                ...
                $(".fetch_cart_value").append(''+value.total_price+'.00');
                // $(".fetch_cart_value_sub").append(''+subPrice+'.00');
                $(".fetch_cart_value_tax").append(''+value.tax+'.00');
    ...
    function loadAllCartCountPay(){ 
     ...
     loadAllCartCountPay();
        $(document).on('click', '.add_to_cart_wear', function(){
            var product_id = $(this).attr("id");
        // alert(product_id);
    // 		var variant_id = $('#variant_id'+product_id).val();
    //        alert(variant_id);
            var product_quantity = $('#quantity').val();
        alert=confirm("Product successfully added to cart");
            var action = "add";
            if(product_quantity > 0)
            {
                $.ajax({
                    url:"ajax/action_wear.php",
                    method:"POST",
